...
public class Utility {
    ...
    public static void parseRules(List<ExecRules> erList, BufferedReader br,
	    Map<Integer, List<ExecRules>> rankRuleLst, boolean isRank, int probVer, 
	...
	try {
		...
		if (probVer == 1) { // only implement first version of prob 
		   ...
		   if (tmp.length() == 0) {
		       System.err.println("No probability exists in rules: " + line);
		       System.exit(-1);
		   }
		 ...
		 * 2. 10: {} 
		 * 3. 10*: {} 
		 * 4. *{ (prob) } no rank, async v 
		 * 5. { (prob) } no rank, sync v 
		 * 6. v1 = v2 + v3; (prob)
		 */
    ...
    public static ExecRules samplingRules(List <Double> accProb, List <ExecRules> orderRuleLst) {
	...
	while (start <= end) {
		...
		start = mid + 1;
	    else return orderRuleLst.get(mid);
	}
