...
public class Utility {
    ...
    public static void parseElements (List <Element> eleList, BufferedReader br, Map<String, Element> eleMap) {
	String line = "";
	try {
	    while ((line = br.readLine()) != null) {
	        ...
	        if (eleMap.containsKey(tmpEle.getName())) {
	            System.err.println("Repeating element: " + tmpEle.getName()); //repeating element
	            System.exit(-1);
	        }
    ...
    public static void parseRules (List <ExecRules> erList, BufferedReader br, Map<Integer, List<ExecRules>> rankRuleLst) {
	String line = "";
	try {
	    while ((line = br.readLine()) != null) {
		line = line.trim();
		...
		if (line.matches("\\**\\{")) {
		    ...
		    if (line.contains("*")) er.setAsync(true);
		    while (!(line = br.readLine()).equals("}")) {
			if (line.length() == 0) continue; //empty line within group
			if (!line.matches(".*\\ =\\ .*;")) {
		    ...
		    erList.add(er);
		} else if (line.matches("[0-9]*\\:.*")) { //case3: 10: v1 = something, normal rule with rank
		    int rank = Integer.parseInt(line.substring(0, line.indexOf(':')).trim());
		    String rule = line.substring(line.indexOf(':') + 1).trim();
		    ExecRules er = new ExecRules(true);
		    er.addRule(new Rule(rule));
		    erList.add(er);
		    List<ExecRules> tmpER = rankRuleLst.get(rank);
		    if (tmpER == null) tmpER = new ArrayList <ExecRules> ();
		    tmpER.add(er);
		    rankRuleLst.put(rank, tmpER);
		} else if (line.matches("[0-9]*\\**\\:\\{")) { //case4 and case5 10:{}/10*:{}	  
		    int rank = 0; 
		    ExecRules er = new ExecRules(false);
		    if (line.contains("*")) {
			er.setAsync(true);
			...
			rank = Integer.parseInt(line.substring(0, line.indexOf(':')).trim());
		    }
		    while (!(line = br.readLine()).equals("}")) {
			if (line.length() == 0) continue; //empty line within group
			if (!line.matches(".*\\ =\\ .*;")) {
		    ...
		    rankRuleLst.put(rank, tmpER);
		    erList.add(er);
		} else if (line.matches(".*\\ =\\ .*")) { //case6 v1 = v2 + v3
		    ExecRules er = new ExecRules(true);
		    er.addRule(new Rule(line));
		    erList.add(er);
		} 
    ...
    public static void caSimulation(int cycles, boolean isRank, List<Element> eleList, List<ExecRules> rule, Map<String, Element> eleMap, Map<Integer, List<ExecRules>> rankRuleLst) {
	//TOTO rank
    ...
    public static void raSimulation(int cycles, List<Element> eleList, List<ExecRules> rule, Map<String, Element> eleMap){
	// no need to implement rank function
	for (int i = 0; i < cycles; i++) { //toggle
	    for (Element ele: eleList) {
		ele.toggle(i);
	    }
	    ExecRules tmpRule = rule.get((int) (Math.random() * rule.size()));
	...
	if (op.equals("!")) return op1 == 1? 0 : 1;
	else if (op.equals("+")) return (op1 + op2) > 0? 1 : 0;
